    ...
    def cfg(file_name, ext="cfg"): # wip
        with open(PATH+file_name+"."+ext, "r") as file:
            ...
            for line in file:
                ...
                try:
                    if "." in line[1]:
                        line[1] = float(line[1]) # float
                    else:
                        line[1] = int(line[1]) # int
                except ValueError:
...
class write(BaseClass):
    ...
    def bin(file_name, data, ext="txt"):
        with open(PATH+file_name+"."+ext, "wb") as file:
            file.write(data)
...
class sql(BaseClass):
    ...
    def new(file_name, ext="db"):
        file = open(PATH+file_name+"."+ext, "w")
        file.close()

    def table(file_name, table, *columns, ext="db"):
        with sqlite3.connect(PATH+file_name+"."+ext) as file:
            c = file.cursor()
            c.execute("CREATE TABLE {} ({})".format(table, ", ".join(columns)))

    def alter(file_name, ext="db"):
        pass

    def insert(file_name, table, ext="db"):
        with sqlite3.connect(PATH+file_name+"."+ext) as file:
            c = file.cursor()
    ...
    def read(file_name, table, cols="*", conditional=None, ext="db"):
        with sqlite3.connect(PATH+file_name+"."+ext) as file:
            c = file.cursor()
            c.execute("SELECT {} FROM {}{}".format(cols, table, "" if conditional == None else " WHERE {}".format(conditional)))
            return c.fetchall()
